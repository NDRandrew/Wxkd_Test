...
# Recomendacoes padrao — uma por linha, cada linha vira um bullet no PDF.
# O usuario pode editar livremente no painel (passo 4).
RECOMENDACOES_DEFAULT: List[str] = [
    "Correcao dos registros inconsistentes e incompletos, priorizando campos criticos.",
    "Revisao das regras de negocio na origem, garantindo que sistemas capturem e validem dados adequadamente.",
    "Implementacao de controles automatizados, como validacoes de formato e preenchimento obrigatorio.",
    "Revisao dos fluxos de ingestao e atualizacao, para minimizar atrasos.",
    "Ajustes no Glossario de Dados, quando aplicavel, garantindo clareza e padronizacao.",
]
...
class ModelDados:
    ...
    def build_context_ns(self, payload: Dict[str, Any], metricas: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Recomendacoes: lista de strings → linhas com prefixo "* " para bullets no PDF
        rec_raw   = payload.get("recomendacoes", RECOMENDACOES_DEFAULT)
        rec_lines = [l.strip() for l in rec_raw if l.strip()] if isinstance(rec_raw, list) else \
                    [l.strip() for l in rec_raw.splitlines() if l.strip()]
...
# Textarea de recomendacoes — uma recomendacao por linha; cada linha vira um bullet no PDF
_w_recomendacoes = W.Textarea(
    value='\n'.join(RECOMENDACOES_DEFAULT),
    placeholder='Uma recomendacao por linha. Cada linha sera exibida como topico no PDF.',
    layout=W.Layout(width='100%', height='140px'),
)
_w_recomendacoes.add_class('pq-textarea')
...
def _build_p4():
    ...
    return W.VBox([
        ...
        W.HTML('<br>'),
        W.HTML('<div class="pq-card-hdr" style="margin-bottom:8px;border-radius:4px;border:1px solid #DCDCDC"><div class="pq-icon">5</div>Recomendacoes</div>'),
        W.HTML('<div class="pq-infobox">Uma recomendacao por linha. Cada linha sera exibida como topico (bullet) na secao 5 do documento. Edite, reordene ou remova linhas livremente.</div>'),
        _w_recomendacoes,
...
def _on_gerar_pdf(b):
    ...
    def _run():
        ...
        payload = {
            ...
            'recomendacoes': _w_recomendacoes.value.splitlines(),
        }
